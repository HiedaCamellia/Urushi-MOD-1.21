...
public class FryingRecipeCategory implements IRecipeCategory<FryingRecipe> {
    public static final ResourceLocation location=ResourceLocation.fromNamespaceAndPath(ModCoreUrushi.ModID,"frying");
    ...
    @Override
    public RecipeType<FryingRecipe> getRecipeType() {
        return JEIUrushiPlugin.JEI_FRYING;
    }

    @Override
    public Component getTitle() {
        return Component.translatable("gui.jei.category.urushi.frying");
    }
    ...
    protected void drawCookTime(FryingRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = 100;
        if (cookTime > 0) {
