...
return false;
    ...
    public static void increaseStoredReiryokuAmount(ItemStack stack, int i) {
        ...
        if (0 <= pre + i && pre + i <= getReiryokuCapacity(stack)) {
            compoundtag.putInt(REIRYOKU_AMOUNT, pre + i);
        } else if (pre + i > getReiryokuCapacity(stack)) {
            compoundtag.putInt(REIRYOKU_AMOUNT, getReiryokuCapacity(stack));
        } else {
            compoundtag.putInt(REIRYOKU_AMOUNT, 0);
        }
        stack.set(DataComponents.CUSTOM_DATA, CustomData.of(compoundtag));
    }
    ...
    public static ParticleOptions getMediumElementParticle(ElementType type) {
        ...
        switch (type) {
            case WoodElement -> particleOptions = (ParticleOptions) ParticleRegister.WoodElementMedium.get();
            case FireElement -> particleOptions = (ParticleOptions) ParticleRegister.FireElementMedium.get();
            case EarthElement -> particleOptions = (ParticleOptions) ParticleRegister.EarthElementMedium.get();
            case MetalElement -> particleOptions = (ParticleOptions) ParticleRegister.MetalElementMedium.get();
            // case WaterElement ->
            default -> particleOptions = (ParticleOptions) ParticleRegister.WaterElementMedium.get();
        }
